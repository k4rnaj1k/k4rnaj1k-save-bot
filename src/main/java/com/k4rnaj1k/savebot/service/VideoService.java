...
public class VideoService {
    ...
    public VideoService(WebClient cobaltWebClient, TelegramClient telegramClient, FileRepository fileRepository, WebClient pubblerWebClient,
                        @Value("${savebot.app.pubblerRetries}")
                        Integer pubblerRetries,
                        @Value("${savebot.app.channelChatId}")
    ...
    private synchronized InputFile getInputFileFromPubbler(String query) {
        ...
        if(Strings.isNotBlank(pubblerJobCreatedResponse.getError())) {
            log.error("Response with error from pubbler for query: {}, error: {}", query, pubblerJobCreatedResponse.getError());
        }
